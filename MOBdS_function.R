...
#new parameters:
#nresample: number of resamples for calculating p-values in splitting criterion
#datenShort: original data with discrete survival (one line per subject)

mobMC <- function(formula, data, subset, na.action, weights, offset, cluster,
                  fit, control = mob_control(),nresample,datenShort, ...)
{
  ...
  #mfShort[[1L]] <- as.data.frame
  #mfShort <- eval(mfShort, parent.frame())
  ...
  n <- nrow(Z)
  nyx <- length(mf) - length(Z) - as.numeric("(weights)" %in% names(mf)) - 
    as.numeric("(offset)" %in% names(mf)) - as.numeric("(cluster)" %in% names(mf))
  ...
  nodes <- mob_partynode(Y = Y, X = X, Z = Z, weights = weights, offset = offset, cluster = cluster,
                         ...
                         datenShort=datenShort,nresample=nresample, ...)
  ...
  rval <- party(nodes, 
                ...
                #datenShort = if(control$model) mfShort else mfShort[0,],
                fitted = fitted,
                terms = mt,
                info = list(
                  call = cl,
...
mob_partynode <- function(Y, X, Z, weights = NULL, offset = NULL, cluster = NULL,
                          fit, control = mob_control(), varindex = 1L:NCOL(Z),datenShort,nresample, ...)
{
  ...
  {  
    ...
    if(vcov != "info") {
      ...
      meat <- if(is.null(cluster)) {
        ...
        ## nclus <- length(unique(cluster)) ## nclus / (nclus - 1L) * 
        crossprod(as.matrix(apply(if(control$caseweights) process/sqrt(weights) else process, 2L, 
                                  tapply, as.numeric(cluster), sum)))
    ...
    ## get critical values for supLM statistic
    from <- if(control$trim > 1) control$trim else ceiling(nobs * control$trim)
    #print(control$minsize)
    #print(from)
    from <- max(from, control$minsize)
    to <- nobs - from
    ...
    #Determine p-value
    perm_stat <- matrix(NA,ncol=dim(z)[2],nrow=nresample)
    for(j in 1:nresample){
      ...
      #print(estfun_perm)
      #print(m_perm)
      z_perm <- datL_new%>%dplyr::select(-"time.discrete",-"state",-"trt",-"y",-"timeInt",-"obj")
      #print(z_perm)
      perm_stat[j,] <- mob_grow_fluctests_woP(estfun= estfun_perm, 
                                              z = z_perm, 
                                              weights=weights,
                                              obj = m_perm,control=control,
                                              minsize=minsize)
    ...
    # print(perm_stat)
    Fn <- sapply(1:dim(z_perm)[2],function(x)ecdf(perm_stat[,x]))
    pval <- if(sum(from_to) > 0L) 1-(sapply(1:dim(z_perm)[2],function(x)Fn[[x]](stat[x])))else NA
    add_Na_pvalue <- function(x,cSpermstat){
      ifelse(cSpermstat[x]==0,
             pval[x] <- NA_real_,pval[x] <- pval[x])
    }
    cSpermstat <- colSums(perm_stat)
    
    pval <- sapply(1:length(pval),add_Na_pvalue,cSpermstat)
    print(pval)
    print(length(which(pval==min(pval,na.rm = T))))
    # select variable with minimal p-value
    if(length(which(pval==min(pval)))==1){
      best <- which.min(pval)}else{
        best <- which.max(stat)
      }
    # best <- which.min(pval)
    
    if(length(best) < 1L) best <- NA
    rval <- list(pval = pval, stat = stat, best = best)
  ...
  mob_grow <- function(id = 1L, y, x, z, weights, offset, cluster,datenShort,nresample, ...)
  {
    if(verbose) {
      if(id == 1L) cat("\n")
    ...
    #print(mod$object)
    test <- if(is.null(mod$estfun)) NULL else try(mob_grow_fluctests(estfun=mod$estfun, z=z, 
                                                                     weights=weights, obj = mod$object, 
                                                                     cluster =cluster,
                                                                     control=control,
                                                                     minsize=minsize,
                                                                     nresample=nresample,
                                                                     datenShort=datenShort) )
    ...
    kidids_short <- kidids_split(sp, data = datenShort%>%dplyr::select(colnames(z)))
    #print(table(kidids_short))
    ## set-up all daugther nodes
    depth <<- depth + 1L
    kids <- vector(mode = "list", length = max(kidids))
    #kidids_short <- vector(mode = "list", length = max(kidids_short))
    for(kidid in 1L:max(kidids)) {
      ## select obs for current next node
      nxt <- kidids == kidid
      #print(nxt%>%table)
      nxt_S <- kidids_short == kidid
      #print(kidids_short%>%table)
      #cat("tab nxt_S")
      #print(nxt_S%>%table)
      ## get next node id
      if(kidid > 1L) {
      ...
      kids[[kidid]] <- mob_grow(id = myid + 1L,
                                y=suby(y, nxt), x=subx(x, nxt), z=subz(z, nxt), weights=weights[nxt], 
                                offset=offset[nxt], cluster=cluster[nxt],
                                datenShort=subz(datenShort, nxt_S),
                                nresample=nresample,
                                ...)
  ...
  ## grow tree
  depth <- 1L
  nodes <- mob_grow(id = 1L, Y, X, Z, weights, offset, cluster,datenShort=datenShort,
                    nresample=nresample, ...)
...
glmtreeMC <- function(formula, data, subset, na.action, weights, offset, cluster,
                      family = gaussian, epsilon = 1e-8, maxit = 25, method = "glm.fit",nresample=100,
                      datenShort, ...)
{
  ...
  m$maxit <- maxit
  m$method <- method
  m$nresample <- nresample
  m$datenShort <- datenShort
